...
function get_stray_quotes($categories=NULL,$sequence=NULL,$linkphrase=NULL,$multi=NULL,$timer=NULL,$noajax=NULL,$myoffset=0,$widgetid=NULL,$fullpage=NULL,$orderby='quoteID',$sort='ASC',$thisid=NULL,$disableaspect=NULL,$contributor=NULL) {
	...
	if ($multi > 1){
		...
		if ($fullpage){
			...
			if ($pages > 1) {
			   $quotepage  = $pages - 1;
			   $prev  = ' <a href="'.$urlpages.$quotepage.'"> Anteriores '.$rows.'</a> | ';		
			   $first = ' <a href="'.$urlpages.'1"> Primero </a> | ';
			}
			else {
			...
			if ($pages < $maxPage) {
				...
				$quotepage = $pages + 1;
				$next = ' | <a href="'.$urlpages.$quotepage.'"> Siguientes '.$missing.'</a> ';
				
				$last = ' | <a href="'.$urlpages.$maxPage.'"> Último </a> ';
			}
			else {
			...
			if ($pages > 1) {
			   $quotepage  = $pages - 1;
			   //$prev  = '<a href="'.$urlpages.$quotepage.'">&laquo; Anteriores '.$rows.'</a>&nbsp;|';		
			}
			else {
			   $prev  = '&nbsp;'; // we're on page one, don't print previous link
			}
			...
			if ($pages < $maxPage) {
				...
				$quotepage = $pages + 1;
				//$next = '&nbsp;<a href="'.$urlpages.$quotepage.'">Próximas '.$missing.' &raquo;</a>';
	...
	if ( !empty($result) ){	
		...
		if ($multi > 1){
			...
			$output .= $beforeAll;
			foreach ( $result as $get_one )
				$output .= '<div align="center" style="margin-top:10px;"'.stray_output_one($get_one,$multi,$disableaspect).'</div>';	
			$output .= $afterAll;
