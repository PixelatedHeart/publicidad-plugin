...
function stray_quotes_options() {
	...
	if(!empty($_POST['do'])) {
		...
		if ($update_quotes_options) {
            ...
            <?php _e('<strong>Opciones salvadas...</strong> ','stray-quotes');
            if ( $msgvar3 == 1) _e('Hemos eliminado alguna publicidad con URL errónea.', 'stray-quotes');
            ...
            else _e('Sin problemas.', 'stray-quotes'); ?></p></div><?php 
            ...
            <?php if ( $msgvar3 == 1 /*|| $msgvar1 == 1|| $msgvar2 == 1*/) 
				_e('La operación falló a causa de URLs erróneas.', 'stray-quotes');
			else 
				_e('<strong>Las opciones no se pueden salvar</strong>. La operación no se realizó correctamente, o no hizo ningún cambio.</strong> ','stray-quotes'); 
			?></p></div><?php 
		}			
    ...
    <?php //quote aspect ?>
    <div class="wrap"><h2><?php _e('Opciones','stray-quotes'); ?></h2>
