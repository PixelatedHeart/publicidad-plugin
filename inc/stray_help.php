<?php function stray_help() {
    ...
    <div class="wrap" style="width:52%"><h2><?php _e('La página de ayuda','stray-quotes');?></h2>
    <!--<span class="setting-description"><?php _e(' ~ where <strong>ico</strong> answers questions, as you click on them.','stray-quotes');?></span>-->
        ...
        <h3 style="cursor:pointer" onclick="expand('text1', this);"><?php _e('¿Cómo publico publicidad aleatoria en el blog?','stray-quotes');?></h3>
        <div id="text1" style="display:none;"><blockquote><p><?php echo str_replace("%s1",$widgetpage,__('El sistema utiliza <strong>Widgets</strong>. Puedes utilizar cuantos widgets necesites para mostrar la publicidad. Lo hemos programado para que, según la categoría en la que estén, se ajusten al tamaño que corresponde. Sólo tienes que añadir el widget de publicidad en el sidebar correspondiente en la <a href="%s1">página de widgets</a> y ajustar la configuración.','stray-quotes')); ?></p></blockquote></div>
     
        <?php // What if I don\'t use widgets?' ?>
        <h3 style="cursor:pointer" onclick="expand('text2', this);"><?php _e('¿Puedo añadir la publicidad directamente?','stray-quotes');?></h3>
        <div id="text2" style="display:none;"><blockquote><p><?php _e('Podrías, pero no hemos preparado el sistema para ello. Si fuera necesario, pídenos una actualización','stray-quotes');?></p>
        
        <p><strong><?php _e('Para obtener una publicidad: ', 'stray-quotes');?>
        ...
        <li>1. <strong><code><?php _e('category1,category2', 'stray-quotes');?></code></strong> : <?php _e('Una lista, separada por comas, de las categorías que utilizaremos para mostrar la publicidad. <code>\'all\'</code> o <code>\'\'</code> usa todas las categorías. Se utilizan todas por defecto', 'stray-quotes');?></li>
