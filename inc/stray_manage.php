...
function stray_manage() {
	...
	if ( $action == 'edit' ) {
		...
		?><div class="wrap"><h2><?php _e('Editar publicidad '.$quoteID, 'stray-quotes') ?></h2><?php 
	
		//check if something went wrong with quote id
		if ( empty($quoteID) ) {
			?><div id="message" class="error"><p><?php _e('No hemos encontrado la ID.','stray-quotes') ?></p></div><?php
		}
            ...
            <form name="quoteform" id="quoteform" method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
				...
				<p><label><?php _e('Nombre: (para identificarla en la lista)','stray-quotes') ?></label><br />
                <div style="float:left; display:none;"><script type="text/javascript">edToolbar();</script></div>
                <div style="float:right; display:compact;margin-top:12px;display:none;"><small><?php _e('Para insertar esta publicidad en algún sitio utiliza:','stray-quotes'); ?> <code>[quote id=<?php echo $quoteID ?>]</code></small></div>
                ...
                <p class="setting-description"><small><?php _e('Si quieres añadir un adsense, escribe "adsense" (sin comillas) en el espacio del enlace','stray-quotes'); ?></small></p></p>
                ...
                <textarea id="seditor" name="quote_source" cols=68 rows=4  <?php echo $styleborder ?> /><?php echo $source ?></textarea>
                ...
                <p class="setting-description"><small></small></p></p>
                
                <p style="display:none;"><label><?php _e('Categoría:&nbsp;','stray-quotes'); ?></label>                
				...
				<p> <a href=" <?php echo $urlaction ?>"><?php _e('Cancelar','stray-quotes') ?></a>&nbsp;
         	   <input type="submit" name="save"  class="button-primary" value="<?php _e('Actualizar publicidad','stray-quotes') ?> &raquo;" /></p>
			</form><p>&nbsp;</p></div><?php 
		...
		?><div class="wrap">
        <h2><?php _e('Gestionar publicidad','stray-quotes') ?></h2><?php 
		
		$nothingmessage = __('Por favor, seleccione algo primero.','stray-quotes');
		$wrongmessage = __('Algo fue mal.','stray-quotes');
		...
		if ( $action == 'edit_save' ) {
			...
			if (preg_match('/\s+/',$category)>0){
				...
				$plusmessage = "<br/>Nota: <strong>El nombre de la categoría creada contiene espacios. Los hemos reemplazado por guiones. Esperamos que no haya problemas por eso.";
			} 
			...
			if ( empty($quoteID) )	{
				?><div id="message" class="error fade"><p><?php _e('<strong>Fallo:</strong> No ha introducido ID.','stray-quotes') ?></p></div><?php
			}
				...
				if ( empty($result) || empty($result[0]->quoteID) )	{			
					...
					<?php echo str_replace("%s",$quoteID,__('Publicidad <strong>%s</strong> actualizada.'.$plusmessage,'stray-quotes'));?></p></div><?php
				}		
			}
		}
		...
		//action: delete quote
			...
			if ( empty($result) || empty($result[0]->quoteID) )	{			
				?><div class="updated"><p><?php echo str_replace("%s",$quoteID,__('Publicidad <strong>%s</strong> borrada.','stray-quotes')); ?></p></div><?php
			}			
			else {						
		...
		//bulk action: delete
			...
			if ($count == 0) { ?><div class="error fade"><?php echo $nothingmessage ?></div><?php
				...
				if ( empty($result) || empty($result[0]->quoteID) )	{			
					?><div class="updated"><p><?php echo str_replace("%s",$count,__('<strong>%s</strong> publicidades borradas.','stray-quotes')); ?></p></div><?php
		...
		//bulk action: toggle visibility
			...
			if ($count == 0) { ?><div class="error fade"><?php echo $nothingmessage ?></div><?php
				...
				if ( $result1 || $result2 )	{			
					?><div class="updated"><p><?php echo str_replace("%s",$count,__('Visibilidad cambiada para <strong>%s</strong> publicidades.','stray-quotes')); ?></p></div><?php
				}			
				else {						
					?><div class="error fade"><?php echo $wrongmessage ?></div><?php	
				}	
		...
		//bulk action: change category
			...
			if ($count == 0) { ?><div class="error fade"><?php echo $nothingmessage ?></div><?php
				...
				if ( $result == $newcat )	{			
					?><div class="updated"><p><?php echo str_replace("%s",$count,__('Categoría modificada para <strong>%s</strong> publicidades.','stray-quotes')); ?></p></div><?php
				}			
				else {						
					?><div class="error fade"><?php echo $wrongmessage ?></div><?php	
				}	
		...
		//bulk action: no action
		 ...
		 ?><div class="error fade"><?php _e('Por favor, seleccione primero algo del menú \'Bulk Actions\'.','stray-quotes'); ?></div><?php 
		}
		...
		if ($pages > 1) {
			...
			$quotepage  = $pages - 1;		
			$prev  = ' <a href="'.querystrings($urlaction, 'qp', $quotepage).'" title=" Anterior '.$rows.'">&laquo;</a> ';		
		...
		if ($pages < $maxPage) {
			...
			$quotepage = $pages + 1;
			$next = ' <a href="'.querystrings($urlaction, 'qp', $quotepage).'" title=" Siguiente '.$missing.'">&raquo;</a> ';
		...
		<option value="multidelete"><?php _e('borrar','stray-quotes'); ?></option>
		<option value="togglevisible"><?php _e('modificar visibilidad','stray-quotes'); ?></option>
		<option value="changecategory"><?php _e('modificar categoría','stray-quotes'); ?></option>
		</select>
		...
		<input type="submit" value="<?php _e('Aplicar','stray-quotes'); ?>" class="button-secondary action" />
		</div>
		...
		<?php  if ( $categories == '' || $categories == 'all' ) echo ' selected'; ?>><?php _e('todas las categorías','stray-quotes'); ?></option>
		...
		echo str_replace($search,$replace,__('<span class="displaying-num">Página %s1 de %s2</span>', 'stray-quotes')); 
		...
		if ( !empty($quotes) ) {
			...
			<thead><tr>
				...
				<?php } else { _e('ID','stray-quotes');
					...
					<img src= <?php echo $imgasc ?> alt="Descendente" title="Descendente" /></a> <?php }
					else if ($sort == 'DESC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'ASC'); ?>">
					<img src= <?php echo $imgdsc ?> alt="Ascendente" title="Ascendente" /></a> <?php } ?>
							
				<?php } }else{ _e('ID','stray-quotes'); }?>            
				</th>
				
				<th scope="col"> <?php _e('Publicidad','stray-quotes') ?> </th>
				
				<th scope="col" style="white-space: nowrap;"> <?php if ($numrows != 1) { if ( $orderby != 'author') { ?>
				<a href="<?php echo querystrings($urlaction, 'qo', 'author'); ?>"><?php _e('Enlace','stray-quotes') ?></a>
				<?php } else { _e('Enlace','stray-quotes');
					if ($sort == 'ASC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'DESC'); ?>">
					<img src= <?php echo $imgasc ?> alt="Descendente" title="Descendente" /></a> <?php }
					else if ($sort == 'DESC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'ASC'); ?>">
					<img src= <?php echo $imgdsc ?> alt="Ascendente" title="Ascendente" /></a> <?php } ?>
				...
				<a href="<?php  echo querystrings($urlaction, 'qo', 'source'); ?>"><?php _e('Imagen','stray-quotes') ?></a>
				<?php } else { _e('Imagen','stray-quotes');
					if ($sort == 'ASC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'DESC'); ?>">
					<img src= <?php echo $imgasc ?> alt="Descendente" title="Descendente" /></a> <?php }
					else if ($sort == 'DESC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'ASC'); ?>">
					<img src= <?php echo $imgdsc ?> alt="Ascendente" title="Ascendente" /></a> <?php } ?>
				...
				<?php } else { _e('Categoría','stray-quotes');
					if ($sort == 'ASC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'DESC'); ?>">
					<img src= <?php echo $imgasc ?> alt="Descendente" title="Descendente" /></a> <?php }
					else if ($sort == 'DESC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'ASC'); ?>">
					<img src= <?php echo $imgdsc ?> alt="Ascendente" title="Ascendente" /></a> <?php } ?>
				...
				<?php } else { _e('Usuario','stray-quotes');
					if ($sort == 'ASC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'DESC'); ?>">
					<img src= <?php echo $imgasc ?> alt="Descendente" title="Descendente" /></a> <?php }
					else if ($sort == 'DESC') { ?><a href="<?php echo querystrings($urlaction, 'qs', 'ASC'); ?>">
					<img src= <?php echo $imgdsc ?> alt="Ascendente" title="Ascendente" /></a> <?php } ?>
			...
			foreach ( $quotes as $quote ) {
				...
				<tr <?php echo($alt); ?> <?php if( $quote->user != $current_user->user_nicename ) echo ' style="color:#aaa"' ?> >
					...
					<td><?php if( $quote->visible == 'yes' ) _e( 'sí', 'stray-quotes' ); else _e( 'no', 'stray-quotes' ); ?></td>
										
					<td align="center">
					<a href="<?php echo querystrings( querystrings($urlaction, 'qa', 'edit'), 'qi', $quote->quoteID ); ?>">
					<?php _e('Editar','stray-quotes') ?></a></td>
	
					<td align="center">
					<a href="
					<?php echo querystrings( querystrings($urlaction, 'qa', 'delete'), 'qi', $quote->quoteID );  ?>"
					onclick="if ( confirm('<?php echo __( 'Va a borrar la publicidad ','stray-quotes') . $quote->quoteID . '.\\n\\\'' . __('Cancelar','stray-quotes') . '\\\' ' . __('para parar','stray-quotes') . ', \\\'OK\\\' ' . __('para borrar','stray-quotes') . '.\''; ?>) ) { return true;}return false;"><?php echo __('Borrar','stray-quotes') ?></a></td>			
		...
		} else { ?><p><div style="clear:both"> <?php echo str_replace("%s1",get_option('siteurl')."/wp-admin/admin.php?page=stray_manage",__('<br/>No hay publicidades aquí. Quizá quieras <a href="%s1">reabrir</a> la página.','stray-quotes')); ?> </div></p>
		</div><?php	}	
	}
