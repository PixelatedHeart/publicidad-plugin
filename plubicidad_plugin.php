<?php
/*
Plugin Name: Publicidad
Plugin URI: http://mecus.es/plugins/
Description: Ads manager. Highly modified stray_quotes plugin to show ads using intervals (50%, 25%)
...
function quotes_activation() {
	...
	//set the messages
	$straymessage = "";
	$newmessage = str_replace("%1","http://www.italyisfalling.com/stray-random-quotes/#changelog",__('<p>Has instalado una nueva versión de <strong>Publicidad</strong>. Todos los cambios están en el <a href="%1">changelog</a>, pero deberías saber: </p>','stray-quotes'));
	...
	if ( !$straytableExists ) {
		...
		`quote`, `author`, `source`) values ('Título de la publicidad.', 'Enlace', 'Imagen') ");
		
		//message
		$straymessage = str_replace("%s1", WP_STRAY_QUOTES_TABLE,__('<p>Acaba de instalar el plugin. Hemos creado la tabla "%s1" en la base de datos para guardar sus publicidades, y hemos añadido una para darle un punto de inicio.</p>','stray-quotes'));
	}
	...
	if (false === $quotesoptions || !is_array($quotesoptions) || $quotesoptions=='' ) {
		...
		$temp = get_option($var);
		if (false === $temp) $quotesoptions[$var] =  '<a rel="external" href="';
		else $quotesoptions[$var] = $temp;
		delete_option($var);		
		unset($var);unset($temp);
		...
		$temp = get_option($var);
		if (false === $temp) $quotesoptions[$var] =  '<img src="';
		else $quotesoptions[$var] = $temp;
		delete_option($var);		
		unset($var);unset($temp);
		$var = 'stray_quotes_after_source';
		$temp = get_option($var);
		if (false === $temp) $quotesoptions[$var] =  '" /></a>';
		else $quotesoptions[$var] = $temp;
		delete_option($var);		
		unset($var);unset($temp);
	...
	*/
	...
	if(  $quotesoptions['stray_quotes_version'] <= 192 ){
		...
		if ($checkuser == false){
			...
			//message
			if (!$straymessage)$straymessage = $newmessage;
			if ($addgroup && $setvalue)$straymessage .= str_replace('%s1',get_option('siteurl')."/wp-admin/admin.php?page=stray_quotes_options",__('<li> Hemos creado una publicidad multi-usuario. Para activar la opción multi-usuario vaya a la <a href="%s1">página de opciones</a>.</li>','stray-quotes'));
	...
	if( $quotesoptions['stray_quotes_version'] <= 195 ){
		...
		//message
		if (!$straymessage)$straymessage = $newmessage;
		$straymessage .= __('<li> por compatibilidad, los códigos han cambiado de nombre. Por favor, tenga en cuenta que el código para el aleatorio es <code>stray-random</code>, para utlizarlas todas el código es <code>stray-all</code> y para mostrar una concreta, <code>stray-id</code>.</li>','stray-quotes');
...
function stray_quotes_add_pages() {
	...
	else $straycan = 'edit_posts';

	add_menu_page('Añadir Publi', __('Añadir Publi','stray-quotes'), $straycan, __FILE__, 'stray_manage', WP_STRAY_QUOTES_PATH.'img/lightbulb.png');
	//add_submenu_page(__FILE__, __('Visión general','stray-quotes'), __('Visión general','stray-quotes'), $straycan, __FILE__, 'stray_intro');
	add_submenu_page(__FILE__, __('Gestionar publicidad','stray-quotes'), __('Gestionar publicidad','stray-quotes'), $straycan, 'stray_manage', 'stray_manage');
	add_submenu_page(__FILE__, __('Añadir nuevo','stray-quotes'), __('Añadir nuevo','stray-quotes'), $straycan, 'stray_new', 'stray_new');
	add_submenu_page(__FILE__, __('Configuración general','stray-quotes'), __('Configuración general','stray-quotes'), 'manage_options', 'stray_quotes_options', 'stray_quotes_options'); 
	add_submenu_page(__FILE__, __('Herramientas','stray-quotes'), __('Herramientas','stray-quotes'), $straycan, 'stray_tools', 'stray_tools');
	add_submenu_page(__FILE__, __('Ayuda','stray-quotes'), __('Ayuda','stray-quotes'), $straycan, 'stray_help', 'stray_help');
	add_submenu_page(__FILE__, __('Borrar','stray-quotes'), __('Borrar','stray-quotes'), 'manage_options', 'stray_remove', 'stray_remove'); 	
